...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for idx in range(0,26,1):
            shifted_lower = lower_first + ((idx + shift) % 26)
            shifted_upper = upper_first + ((idx + shift) % 26)
            word_lookup[chr(lower_first + idx)] = chr(shifted_lower)
            word_lookup[chr(upper_first + idx)] = chr(shifted_upper)
    ...
    def apply_shift(self, shift):
        ...
        shifted = ''.join(map(lambda x: shift_dict[x] if x in shift_dict else x, self.message_text))
        return shifted
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift_reverse in range(0,-26,-1):
            shifted_text = self.apply_shift(shift_reverse)
            splitted_words = shifted_text.split()

            valid_word_count = sum(map(lambda x: 1 if is_word(words, x) else 0, splitted_words))
            if valid_word_count > best_match_count:
                best_match_word = shifted_text
                best_match_count = valid_word_count
                shift_value = 26 + shift_reverse

        return (shift_value, best_match_word)
